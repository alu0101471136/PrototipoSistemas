...
public class HunterAgent : Agent {
  ...
  public override void OnEpisodeBegin() {
    ...
    // Reiniciar la posicion de todas las presas
    foreach (var prey in preysInsidePlayground) {
      float randomXPrey = UnityEngine.Random.Range(floorLimits[0].position.x, floorLimits[1].position.x);
      float randomYPrey = UnityEngine.Random.Range(floorLimits[0].position.y, floorLimits[1].position.y);
      prey.transform.position = new Vector3(randomXPrey, randomYPrey, 0f);
    }

    // Reiniciar el sprite del suelo
    floorSpriteRenderer.material.color = Color.white;
  }
  ...
  public void OnCollisionEnter2D(Collision2D collision) {
    if (collision.collider.CompareTag("Prey")) {
      ...
      floorSpriteRenderer.sortingOrder = 0;
      EndEpisode();
